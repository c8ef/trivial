        ...
        if (auto alloc = dyn_cast<AllocaInst>(inst)) {
          ...
          for (auto use = alloc->uses.head; use; use = use->next) {
            if (auto store = dyn_cast<StoreInst>(use->user)) {
              if (auto data = dyn_cast<ConstValue>(store->data.value),
                  index = dyn_cast<ConstValue>(store->index.value);
                  data && index) {
          ...
          if (can_make_global) {
            ...
            for (inst_check = next_inst(alloc, bb_check); inst_check;
                 inst_check = next_inst(inst_check, bb_check)) {
