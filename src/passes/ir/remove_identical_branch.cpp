...
void remove_identical_branch(IrFunc* f) {
  for (BasicBlock* bb = f->bb.head; bb; bb = bb->next) {
    ...
    if (!i2 || !isa<ParamRef>(i2->arr.value) ||
        i2->arr.value == i1->arr.value || not_singly_used(i2))
      continue;
    auto i3 = dyn_cast<BinaryInst>(i2->next);
    if (!i3 || i3->tag != Value::Tag::Mul || i3->lhs.value != cond->lhs.value ||
        i3->rhs.value != i2 || not_singly_used(i3))
      continue;
    auto i4 = dyn_cast<BinaryInst>(i3->next);
    if (!i4 || i4->tag != Value::Tag::Add || i4->lhs.value != i1 ||
        i4->rhs.value != i3 || not_singly_used(i4))
      continue;
    auto i5 = dyn_cast<StoreInst>(i4->next);
    if (!i5 || i5->arr.value != i1->arr.value || i5->next != j2) continue;
    DEBUG("Performing remove identical branch");
