...
void mem2reg(IrFunc* f) {
  ...
  std::unordered_map<Value*, u32>
      alloca_ids;  // 把alloca映射到整数，后面有好几个vector用这个做下标
      ...
      for (Inst* i = bb->insts.head; i;) {
        ...
        if (auto it = alloca_ids.find(i); it != alloca_ids.end()) {
          ...
          if (it != alloca_ids.end()) {
            ...
            bb->insts.remove(x);
            x->arr.value =
                nullptr;  // 它用到被delete的AllocaInst，已经不能再访问了
            delete x;  // 跟i->deleteValue()作用是一样的(但是跟delete
                       // i不一样)，逻辑上节省了一次dispatch的过程
          }
          ...
          auto it = phis.find(
              x);  // 也许程序中本来就存在phi，所以phis不一定包含了所有的phi
          if (it != phis.end()) {
            values[it->second] = x;
          }
        }
