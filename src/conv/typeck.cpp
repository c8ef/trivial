...
struct Symbol {
  ...
  static Symbol mk_func(Func* f) {
    return Symbol{reinterpret_cast<size_t>(f) | 1U};
  }

  static Symbol mk_decl(Decl* d) {
    return Symbol{reinterpret_cast<size_t>(d) | 2U};
  }

  [[nodiscard]] Func* as_func() const {
    return (p & 1U) ? reinterpret_cast<Func*>(p ^ 1U) : nullptr;
  }

  [[nodiscard]] Decl* as_decl() const {
    return (p & 2U) ? reinterpret_cast<Decl*>(p ^ 2U) : nullptr;
  }
};
...
struct Env {
  ...
  void ck_stmt(Stmt* s) {
    if (auto x = dyn_cast<Assign>(s)) {
      ...
      if (!((cur_func->is_int && is_int(t)) ||
            (!cur_func->is_int && !t.first))) {
